...
collect a class's results read "1 reports", and a set of a single piece offered to
"Show the 1 pieces".
...
**A note that moved you on is scored as you played it.** With the forgiving setting
on, the note that carried you into the next position was recorded as if nobody had
struck it, and its loudness was marked as silence. It carries its own loudness now,
and a note the app credited without hearing stays out of the expression reading
altogether.

**A first-time bar longer than two bars plays as written.** The bars in the middle
of a longer first ending were played on every pass, so a three-bar ending came
round again the second time through. The whole bracket is skipped now.

**A chromatic scale in a flat or sharp key starts on its own tonic.** The E-flat
chromatic scale began on E natural and spelled its tonic as D sharp; it starts on
E flat, and every octave is written as E flat.

**A tremolo leaves the other hand's note alone.** A rocking tremolo in the left
hand also rocked whatever the right hand struck at the same moment, so a melody
note was repeated eight times a bar. Only the chord that carries the mark is
rocked.

**Colouring the notes while the sheet is redrawing takes.** Flipping the colour
...
and out comes the piano exactly as the video export sounds it. The format is
whatever your browser makes best, so there is nothing to choose and nothing that
fails to work.
...
a desktop, the icon was a small tile marooned in a white circle with a band of white
all round it. Launchers crop an icon to their own shape and fill whatever is left
with their own colour, so Plinky now ships an icon made for that: the violet reaches
every edge and the keys sit in the middle, whatever shape your launcher cuts it to.
Reinstall to pick it up.

**Links to a piece work properly when you share them.** Paste one into a chat or a
post and it shows a preview card instead of nothing, and search engines can find the
pieces at last — the site had been telling them every piece in the catalogue was a
...
The piece's name and the composer under it are both set in Plinky's own lettering, and
the licence gets a line of its own written out in full — "CC0 1.0 Universal Public
Domain Dedication" rather than a code. The credit used to run along the bottom edge,
where the falling notes crossed it and the keys washed it out. The Plinky mark in the
corner is the same one the app wears, plink and all. The progress bar is gone: the
notes falling already tell you where you are, and the bar was saying it twice. Nothing
about a piece changes; it is the same music, credited where you can read it.
...
you'll see what it comes out at with its inner notes taken out — a Grade 8 Chopin
mazurka that is Grade 5 with the melody alone, a Grade 5 sonata that is Grade 3
outlined. 826 pieces have one. It is measured with the same model that grades
everything else, so the number under the chip means what the number on it means.

**Thinning a piece is on the play screen too, not only in Settings.** It sits with the
other reading aids — colour, the notes highway, forgiving — because "can I manage this
one today" is a question you ask at the keyboard. Whichever place you set it, it is
the same choice: a piece you thinned while playing stays thinned until you turn it
back.
...
work, so nearly half the tiles had piled into grade 8 together. They spread across
the eight grades again, the studies sit between grades 1 and 6, and every exercise
draws its opening notes on its row the way pieces do.

**A grade now means the same thing every day.** Until now a piece's grade was its
place in a queue: Plinky sorted the whole catalogue by difficulty and cut it into
eight equal piles, so a grade meant "in the easiest eighth of everything here so
far" — and every time the catalogue grew, pieces you had already worked on quietly
changed grade underneath you. Grades are now fixed marks on a scale, set against
teaching collections whose level is settled: Burgmüller's easy studies, the Anna
Magdalena minuets, Bach's two-part inventions, the Chopin études. A piece keeps its
grade whatever else arrives, and grade 3 means grade 3.

**Difficulty now knows how much time you have.** A wide left-hand stretch was
costed the same whether you had a whole beat to make it or half of a sixteenth,
which is why Satie's Gymnopédies — slow, spacious, and within reach in your first
years — were sitting at grade 8, above every Chopin étude. Reading how far apart
the notes fall puts them at grade 4, where a teacher would. Difficulty also reads
the key signature and how far each hand ranges now, so four sharps and a hand that
travels count for something. Most of the catalogue has moved, and famous pieces
land close to where the graded syllabuses put them.

**Suggested fingering follows the tempo too.** The fingering strip used to solve
every leap as if it had to happen instantly, contorting a hand to avoid a jump you
had a full beat for. It now weighs the clock alongside the stretch, and the
green/amber/red marks judge your choice under the same terms as the advice.

## 27 August 2026 — later

**More pieces, and each one says where it came from.** The catalogue has grown by
about two hundred, and every piece imported from now on records its source and
whether it is solo piano, a song with piano, or a choral setting. The licence line
under a piece can finally credit the right place — several hundred pieces were
quietly crediting the wrong one — and the practice ladder can offer piano writing
without steering a beginner into a Schubert accompaniment.

**German note names.** German notation calls B natural **H** and keeps **B** for B
flat, so a scale Plinky called "B-Dur" was asking a German reader to play the wrong
one. Scale and arpeggio names on the German site now use the note names German
musicians actually write, accidentals included — Fis, Es, As. The other languages
that share this convention, among them Polish, Czech, Danish and Swedish, still read
letters: each spells its accidentals its own way, and guessing would swap one wrong
name for another.

**The warm-up card speaks your language.** The scale it offers was named in English
whatever language the rest of the page was in.
...
screen said nothing had changed. What has landed is now put back, so a restore that
cannot finish leaves the device exactly as it was. In the rare case where even that
is refused, the message says so plainly instead.
...
draw every note as its own shape — the old American hymn tunes, Billings and Holden
and Read — one shape per step of the scale, so a singer finds the tune without working
out the key first. The glossary explains them now, and a piece that uses them says so
before you start, like every other mark it asks you to read. They sound exactly like
round notes: the shape tells you where you are, not what to play.

**Five more pieces have gone, because nobody wrote them.** Their composer read "Misc
Christmas" or "Misc Traditional" — a filing bucket from the sheet music we harvest,
sitting where a name should be. The music was old enough to be free, but a piece
whose composer is a category has no page to belong to and tells you nothing about who
made it. Anything credited that way is now turned away at the door.

**Two composers who are one person can no longer slip through.** Burgmüller had three
pages for months because nothing checked. Every pair of composer pages that might be
the same person is now held against a list of the ones we have looked at and ruled on
— Mozart and his father, both Schumanns, Weber and Webern, all genuinely two people —
and a new pair nobody has ruled on stops the build until somebody does.
...
far you have got, and on a loop it used to still be there from the pass before — so
the second time through, the bars were already coloured and told you nothing. Each
pass starts clean.

**The recorded-piano panel explains its second number.** Having all 637 recordings on
the device while it said "43 ready to play" read like something had gone wrong.
Nothing had: recordings load as a piece asks for them, and that count starts from
nothing each visit. It now says so.
...
swaps. The drill's eleven settings fold away behind **Shape the drill** until you want
them.
...
arrived. There is a button to fetch the whole instrument at once — worth pressing
before a flight, it is about 85 MB — and one to delete the recordings and take the
space back without switching the recorded piano off.
...
whichever button matters most on the screen you are looking at. The little drawings
beside the greeting brighten with it: the morning sun is sunlight rather than bronze.

**The ways to practise look like the rest of the app.** The six cards on the front page
now match the blocks on Learn — bordered, lifted off the page, each with its own icon —
and each sounds a note as your mouse crosses it, so running an eye down the six plays a
scale.
...
methods is written in plainer words than before, leads with why it works, says what
in Plinky does it, and carries a button
...
brown-black, and violet is kept for the logo and for the button you are meant to reach
for. Scores still print as ink on light paper in both themes.
...
kilobytes each, and the switch is still there if you would rather Plinky made the
sound itself.

**The piano is standing in a room now.** Every note used to go straight out with no
space around it at all, which is the one thing no real piano ever does. There is a
small hall around it — enough to hear, well short of a wash — and it costs nothing to
download.

**The recorded piano knocks and rings like a real one.** Turn on the grand piano and
you now hear the key coming back up and the damper landing, and — when the sustain
pedal is down, whether you are holding it or the score asks for it — the other strings
answering the one you played. Both were already in the recordings and neither was ever
played.
...
Listen or the play-along running underneath, sounding the piece with nothing on screen
to stop it. It stops now.

**Practice takes over from Listen at any pace.** With "Keep up" chosen, pressing
Practice while Listen was playing did nothing at all — no run, no reason given. It now
takes the piece over, the way it always did at your own pace.

**"Keep going" is gone from the tempo-locked pace, where it never meant anything.** A
run on the clock does not wait for you, so a missed note could never have stopped you
there — the switch had nothing to change.
...
was written. They now sound, in the key the piece is in: a trill in E flat reaches for
the B flat, not the B.
...
It matters most in Settings, where the backup and the reset live: those are the
things you need reachable precisely when something has gone wrong.

**Plinky names the chord you just played.** Put your hands down anywhere on the
keyboard on the home page — computer keys, the on-screen keys, or a piano plugged
in — and it says what you found: the note, the interval, or the chord, with the
bass after a slash where you have played an inversion. It is the one thing you
cannot look up, because you cannot search for a sound you have no name for.

**A new tool for what happens between two chords.** The chord panel says what is in
a chord; this one says why moving from one to another falls under the hand or
fights it — the notes both chords hold on to, and how far the rest have to travel.
On the tools page, under "Between two chords".

**Any chord or scale can be saved as a picture.** The keyboard diagram on the tools
page now has a "Save as picture" button, with the notes marked and the white keys
named — for a lesson plan, a printout, or sending to somebody who is stuck.
