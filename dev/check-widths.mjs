...
// A page is measured as it loads, and an ear-training question does not exist until the
// player starts one: its row of listening dots and its row of answer slots are built per
// question, and both grow with the level. So the widest of each is started and measured
// too. Five notes to write down is the longest melody (MELODIC_LEVELS in
// core/earExercise.ts), and the fourth progression level puts every chord on the keypad
// (PROGRESSION_LEVELS). The page answers a level it does not have with the first one, so
// each question also names what it must show — `count` elements matching `selector` — and
// a question that comes out smaller fails rather than passing as the easy level.
const QUESTIONS = [
    { exercise: "melodic-dictation", level: 2, selector: "fieldset > div", count: 5 },
    { exercise: "progressions", level: 3, selector: "button[aria-keyshortcuts]", count: 7 },
];
...
// The ear page's start button, named in the language being measured.
const START = JSON.parse(readFileSync(`messages/${LOCALE}.json`, "utf8")).ear_start;

// How far the page reaches past the screen, and what is doing it, or null when it fits.
function measure(page) {
    return page.evaluate(() => {
        ...
        const over = all.filter((node) => node.scrollWidth > doc.clientWidth || reaches(node));
...
const report = (width, where, over) => {
    ...
    problems.push(`${width}px ${where}: ${over.spill}px past the screen — ${blame}`);
};
...
try {
    for (const width of WIDTHS) {
        ...
        for (const path of paths) {
            ...
            report(width, path, await measure(page));
        }
        for (const { exercise, level, selector, count } of QUESTIONS) {
            const where = `/ear?exercise=${exercise}&level=${level}`;
            const url = `${pageUrl(`http://localhost:${port}`, LOCALE, "/ear")}?exercise=${exercise}&level=${level}`;
            await page.goto(url, { waitUntil: "domcontentloaded" });
            await page.getByRole("button", { name: START, exact: true }).click();
            // The keypad is the last thing a question builds: once its number keys are
            // there, so are the dots and the slots.
            await page.waitForSelector("[aria-keyshortcuts]", { timeout: 15_000 });
            await page.waitForTimeout(600);
            const shown = await page.locator(selector).count();
            if (shown !== count) {
                problems.push(
                    `${width}px ${where}: expected ${count} of \`${selector}\`, found ${shown} — ` +
                        "the level no longer exists, so this measured an easier question",
                );
            }
            report(width, `${where} (a question in progress)`, await measure(page));
        }
        await page.close();
        console.log(`  ${width}px — ${paths.length} pages, ${QUESTIONS.length} ear questions`);
    }
} finally {
    await browser.close();
    await close();
...
console.log(`\nEvery page fits ${WIDTHS.join(", ")}px, a question in progress included.`);
