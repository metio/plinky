...
import { layered, themeColours } from "./themeColours.mts";

// Every note name the on-screen keyboard can print, measured against every fill it can sit
// on: each skin, both key colours, every state, every palette in every shade, bare and
// under the hold bar. The a11y sweep only sees the keys a page happens to render at rest,
// in the palette it was run in, and axe does not score a one-letter name at all — so a
// label under the floor on a lit key, a pastel skin, another palette or the black mode is
// invisible to it, and this is where such a token change fails.
...
for (const look of colours.looks) {
    const [hold] = colours.fills(look, HOLD_FILL);
    for (const skin of KEYBOARD_THEMES) {
        ...
        for (const colour of ["white", "black"] as const) {
            for (const state of STATES) {
                ...
                const ink = colours.ink(look, face.label);
                for (const fill of colours.fills(look, face.fill)) {
                    const where = `${look.palette} ${look.shade} ${skin.id} ${colour} key, ${state}: ${ink.name} on ${fill.name}`;
...
describe("the note names on the keyboard", () => {
    ...
    it("measures every skin, both key colours and every state, in every palette and shade", () => {
        ...
        // parser did not read.
        expect(colours.looks.length).toBeGreaterThanOrEqual(6);
        expect(pairings.length).toBeGreaterThanOrEqual(
            colours.looks.length * KEYBOARD_THEMES.length * 2 * STATES.length * 2,
        );
    });
    ...
    it("would have caught the pale grey the names were printed in", () => {
        ...
        const pale = colours.token(look, "gray-400");
        const white = colours.token(look, "key-white");
