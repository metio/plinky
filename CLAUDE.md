...
npm run mark          # writes the vector mark, brand/mark/*.svg, with the name outlined
                      # from the Fredoka the app ships; `-- --check` is the blocking gate
npm run icons         # regenerates public/ icons + favicon + og.png from brand/mark (not a gate)
...
- **Stories are visual regression tests, in both themes.** The `storybook`
  ...
  machines on an unrelated commit. The size gate, both a11y sweeps and
  `ci-lighthouse` all build the heaviest locale, which compiles `app/paraglide/`
  for Greek while they run; `npm run build`'s `postbuild` step recompiles the full
  catalogue afterwards, so the tree is never left pinned — a baseline or a jsdom
  test taken straight after one of those gates reads English again. The one way
  to still land on Greek is a pinned build killed before its `postbuild`; `npm run
  messages` puts it right. Stories must render
...
- **A grade is a fixed mark, not a place in the queue.** `GRADE_THRESHOLDS.piece` in
  ...
  reduction costs in `dev/catalog-reach-costs.json`, `songs:bake` grades those against the
  boundaries exactly as it grades `cost`, and the probes remeasure them, so a boundary move
  re-grades the ways in along with the pieces. The costs stay out of the manifest because
  only the bake reads them, and every browsing visitor downloads the manifest.
