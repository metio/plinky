...
// per locale, marked by a `lang` field (what the Studio's language dropdown writes)
// or by the entry's `_key` (what our seed imports set); the query accepts either
// and falls back to English, so a visitor downloads only their locale's words, not
// all 26. The picture is shared across languages — only the text is translated.
