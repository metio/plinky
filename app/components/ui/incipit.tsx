...
import { type Incipit, layoutIncipit } from "../../../core/incipit";
import { BOOMWHACKER_SET } from "../../../core/pitchColor";

// One staff space, in pixels. Every other measurement is a multiple of it, so the whole
// mark rescales by changing this one number and nothing drifts out of proportion.
const SPACE = 6;
...
function Accidental({ x, y, alter }: { x: number; y: number; alter: number }) {
    if (alter > 0) {
        return (
            ...
            {glyphs.map((glyph) => {
                ...
                return (
                    <g key={glyph.slot}>
                        ...
                        {glyph.alter !== 0 && (
                            <Accidental x={x - 1.3 * SPACE} y={y} alter={glyph.alter} />
                        )}
                        <ellipse
                            cx={x}
                            cy={y}
