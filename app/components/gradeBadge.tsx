...
// each grade's pool the player has mastered, so it resolves after mount (nothing on
// the server, matching the first client render) and refreshes on the practice event.
// Hidden until grade 1.
    ...
    useEffect(() => {
        ...
        const read = () => {
            loadGradedMastery().then((items) => {
                if (!cancelled) {
                    setLevel(currentGrade(items, "gentle", Date.now()));
                }
            });
        };
        read();
