...
// A recording to play, at the speed that pitches it, and the share of the note it carries
// (a power; 1 when it is the whole note). `blend` is the neighbouring layer's recording
// carrying the rest, where the force landed near the boundary between two layers.
export type SampleVoice = {
    buffer: AudioBuffer;
    rate: number;
    share?: number;
    blend?: { buffer: AudioBuffer; rate: number; share: number };
};
